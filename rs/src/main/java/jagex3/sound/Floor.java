...
public class Floor {
	...
	@ObfuscatedName("e.r")
	public static final int[] rangeVector = new int[] { 256, 128, 86, 64 };

	// jag::oldscape::sound::Floor::m_inverseDBTable
	@ObfuscatedName("e.d")
	public static final float[] inverseDBTable = new float[] {
	...
	public static int lowNeighbour(int[] arg0, int arg1) {
		int var2 = arg0[arg1];
		int var3 = -1;
	...
	public static int highNeighbour(int[] arg0, int arg1) {
		int var2 = arg0[arg1];
		int var3 = -1;
	...
	public int renderPoint(int arg0, int arg1, int arg2, int arg3, int arg4) {
		int var6 = arg3 - arg1;
		int var7 = arg2 - arg0;
	...
	public void renderLine(int x0, int y0, int x1, int y1, float[] d, int arg5) {
		int dy = y1 - y0;
		int adx = x1 - x0;
		int ady = dy < 0 ? -dy : dy;
		int base = dy / adx;
	...
	public Floor() {
		int floor_types = JagVorbis.readBits(16);
		if (floor_types != 1) {
			throw new RuntimeException();
		}

		int partitions = JagVorbis.readBits(5);
		int max_class = 0;
		this.partition_class_list = new int[partitions];

		for (int i = 0; i < partitions; i++) {
			int value = JagVorbis.readBits(4);
		...
		for (int i = 0; i < max_class; i++) {
			this.class_dimensions[i] = JagVorbis.readBits(3) + 1;
			int subs = this.class_subclasses[i] = JagVorbis.readBits(2);

			if (subs != 0) {
				this.class_masterbooks[i] = JagVorbis.readBits(8);
			}
			...
			for (int j = 0; j < subclass_count; j++) {
				subclass[j] = JagVorbis.readBits(8) - 1;
			}
		}

		this.floor1_multiplier = JagVorbis.readBits(2) + 1;
		int rangebits = JagVorbis.readBits(4);
		...
		for (int i = 0; i < partitions; i++) {
			...
			for (int j = 0; j < this.class_dimensions[partition]; j++) {
				this.Xlist[values++] = JagVorbis.readBits(rangebits);
			}
		}
	...
	public boolean packetDecode() {
		...
		post[0] = JagVorbis.readBits(var5);
		post[1] = JagVorbis.readBits(var5);
		...
		for (int var7 = 0; var7 < this.partition_class_list.length; var7++) {
			...
			for (int var13 = 0; var13 < var9; var13++) {
				...
				var12 >>>= var10;
				post[var6++] = var14 >= 0 ? JagVorbis.codebooks[var14].decodeScalar() : 0;
			}
		}
		return true;
	}
	...
	public void synthMul(float[] arg0, int arg1) {
		...
		float var21 = inverseDBTable[var17];

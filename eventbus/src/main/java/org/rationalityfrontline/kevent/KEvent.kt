...
class KEvent(
    ...
    init {
        scope.launch {
            eventChannel.consumeAsFlow().collect { event ->
                val subscriberList = subscribersReadOnlyMap[event.data.javaClass]
    ...
    ): Boolean = post(Event(data, dispatchMode, isSticky))
    ...
    fun post(instance: Any) {
        this.post(instance, defaultDispatchMode, false)
    }
}
...
public class Component {
	...
	public Pix32MapView[] invSlotSprite;
	...
	public static void unpack(@OriginalArg(3) Jagfile jag, @OriginalArg(0) Jagfile media, @OriginalArg(1) PixFontMapView[] fonts) {
		...
		while (dat.pos < dat.data.length) {
			...
			if (com.type == TYPE_INV) {
				...
				com.invSlotSprite = new Pix32MapView[20];
	...
	private static Pix32MapView getImage(@OriginalArg(0) Jagfile media, @OriginalArg(2) String sprite, @OriginalArg(1) int spriteId) {
		@Pc(8) long uid = (JString.hashCode(sprite) << 8) + (long) spriteId;
		@Pc(13) Pix32MapView image = (Pix32MapView) imageCache.get(uid);
		...
		try {
			image = new Pix32MapView(media, sprite, spriteId);

package jagex2.dash3d;

import deob.ObfuscatedName;
import jagex2.datastruct.Linkable;

public class Ground {

	@ObfuscatedName("JQCVNYYR.l")
	public boolean field912 = true;

	@ObfuscatedName("JQCVNYYR.m")
	public int field913;

	@ObfuscatedName("JQCVNYYR.n")
	public int field914;

	@ObfuscatedName("JQCVNYYR.o")
	public int field915;

	@ObfuscatedName("JQCVNYYR.p")
	public int field916;

	@ObfuscatedName("JQCVNYYR.a")
	public int field901;

	@ObfuscatedName("JQCVNYYR.b")
	public int[] field902;

	@ObfuscatedName("JQCVNYYR.c")
	public int[] field903;

	@ObfuscatedName("JQCVNYYR.d")
	public int[] field904;

	@ObfuscatedName("JQCVNYYR.h")
	public int[] field908;

	@ObfuscatedName("JQCVNYYR.i")
	public int[] field909;

	@ObfuscatedName("JQCVNYYR.j")
	public int[] field910;

	@ObfuscatedName("JQCVNYYR.e")
	public int[] field905;

	@ObfuscatedName("JQCVNYYR.f")
	public int[] field906;

	@ObfuscatedName("JQCVNYYR.g")
	public int[] field907;

	@ObfuscatedName("JQCVNYYR.k")
	public int[] field911;

	@ObfuscatedName("JQCVNYYR.q")
	public static int[] field917 = new int[6];

	@ObfuscatedName("JQCVNYYR.r")
	public static int[] field918 = new int[6];

	@ObfuscatedName("JQCVNYYR.s")
	public static int[] field919 = new int[6];

	@ObfuscatedName("JQCVNYYR.t")
	public static int[] field920 = new int[6];

	@ObfuscatedName("JQCVNYYR.u")
	public static int[] field921 = new int[6];

	@ObfuscatedName("JQCVNYYR.v")
	public static int[] field922 = new int[] { 1, 0 };

	@ObfuscatedName("JQCVNYYR.w")
	public static int[] field923 = new int[] { 2, 1 };

	@ObfuscatedName("JQCVNYYR.x")
	public static int[] field924 = new int[] { 3, 3 };

	@ObfuscatedName("JQCVNYYR.y")
	public static final int[][] field925 = new int[][] { { 1, 3, 5, 7 }, { 1, 3, 5, 7 }, { 1, 3, 5, 7 }, { 1, 3, 5, 7, 6 }, { 1, 3, 5, 7, 6 }, { 1, 3, 5, 7, 6 }, { 1, 3, 5, 7, 6 }, { 1, 3, 5, 7, 2, 6 }, { 1, 3, 5, 7, 2, 8 }, { 1, 3, 5, 7, 2, 8 }, { 1, 3, 5, 7, 11, 12 }, { 1, 3, 5, 7, 11, 12 }, { 1, 3, 5, 7, 13, 14 } };

	@ObfuscatedName("JQCVNYYR.z")
	public static final int[][] field926 = new int[][] { { 0, 1, 2, 3, 0, 0, 1, 3 }, { 1, 1, 2, 3, 1, 0, 1, 3 }, { 0, 1, 2, 3, 1, 0, 1, 3 }, { 0, 0, 1, 2, 0, 0, 2, 4, 1, 0, 4, 3 }, { 0, 0, 1, 4, 0, 0, 4, 3, 1, 1, 2, 4 }, { 0, 0, 4, 3, 1, 0, 1, 2, 1, 0, 2, 4 }, { 0, 1, 2, 4, 1, 0, 1, 4, 1, 0, 4, 3 }, { 0, 4, 1, 2, 0, 4, 2, 5, 1, 0, 4, 5, 1, 0, 5, 3 }, { 0, 4, 1, 2, 0, 4, 2, 3, 0, 4, 3, 5, 1, 0, 4, 5 }, { 0, 0, 4, 5, 1, 4, 1, 2, 1, 4, 2, 3, 1, 4, 3, 5 }, { 0, 0, 1, 5, 0, 1, 4, 5, 0, 1, 2, 4, 1, 0, 5, 3, 1, 5, 4, 3, 1, 4, 2, 3 }, { 1, 0, 1, 5, 1, 1, 4, 5, 1, 1, 2, 4, 0, 0, 5, 3, 0, 5, 4, 3, 0, 4, 2, 3 }, { 1, 0, 5, 4, 1, 0, 1, 5, 0, 0, 4, 3, 0, 4, 5, 3, 0, 5, 2, 3, 0, 1, 2, 5 } };

	public Ground(int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11, int arg12, int arg13, int arg14, int arg15, int arg16, int arg17, int arg18, int arg19) {
		if (arg3 != arg13 || arg2 != arg3 || arg0 != arg3) {
			this.field912 = false;
		}
		this.field913 = arg12;
		this.field914 = arg19;
		this.field915 = arg16;
		this.field916 = arg7;
		short var21 = 128;
		int var22 = var21 / 2;
		int var23 = var21 / 4;
		int var24 = var21 * 3 / 4;
		if (arg10 < this.field901 || arg10 > this.field901) {
			throw new NullPointerException();
		}
		int[] var25 = field925[arg12];
		int var26 = var25.length;
		this.field902 = new int[var26];
		this.field903 = new int[var26];
		this.field904 = new int[var26];
		int[] var27 = new int[var26];
		int[] var28 = new int[var26];
		int var29 = arg4 * var21;
		int var30 = arg18 * var21;
		for (int var31 = 0; var31 < var26; var31++) {
			int var44 = var25[var31];
			if ((var44 & 0x1) == 0 && var44 <= 8) {
				var44 = (var44 - arg19 - arg19 - 1 & 0x7) + 1;
			}
			if (var44 > 8 && var44 <= 12) {
				var44 = (var44 - 9 - arg19 & 0x3) + 9;
			}
			if (var44 > 12 && var44 <= 16) {
				var44 = (var44 - 13 - arg19 & 0x3) + 13;
			}
			int var45;
			int var46;
			int var47;
			int var48;
			int var49;
			if (var44 == 1) {
				var45 = var29;
				var46 = var30;
				var47 = arg3;
				var48 = arg11;
				var49 = arg1;
			} else if (var44 == 2) {
				var45 = var22 + var29;
				var46 = var30;
				var47 = arg3 + arg13 >> 1;
				var48 = arg8 + arg11 >> 1;
				var49 = arg1 + arg17 >> 1;
			} else if (var44 == 3) {
				var45 = var21 + var29;
				var46 = var30;
				var47 = arg13;
				var48 = arg8;
				var49 = arg17;
			} else if (var44 == 4) {
				var45 = var21 + var29;
				var46 = var22 + var30;
				var47 = arg2 + arg13 >> 1;
				var48 = arg5 + arg8 >> 1;
				var49 = arg9 + arg17 >> 1;
			} else if (var44 == 5) {
				var45 = var21 + var29;
				var46 = var21 + var30;
				var47 = arg2;
				var48 = arg5;
				var49 = arg9;
			} else if (var44 == 6) {
				var45 = var22 + var29;
				var46 = var21 + var30;
				var47 = arg0 + arg2 >> 1;
				var48 = arg5 + arg6 >> 1;
				var49 = arg9 + arg14 >> 1;
			} else if (var44 == 7) {
				var45 = var29;
				var46 = var21 + var30;
				var47 = arg0;
				var48 = arg6;
				var49 = arg14;
			} else if (var44 == 8) {
				var45 = var29;
				var46 = var22 + var30;
				var47 = arg0 + arg3 >> 1;
				var48 = arg6 + arg11 >> 1;
				var49 = arg1 + arg14 >> 1;
			} else if (var44 == 9) {
				var45 = var22 + var29;
				var46 = var23 + var30;
				var47 = arg3 + arg13 >> 1;
				var48 = arg8 + arg11 >> 1;
				var49 = arg1 + arg17 >> 1;
			} else if (var44 == 10) {
				var45 = var24 + var29;
				var46 = var22 + var30;
				var47 = arg2 + arg13 >> 1;
				var48 = arg5 + arg8 >> 1;
				var49 = arg9 + arg17 >> 1;
			} else if (var44 == 11) {
				var45 = var22 + var29;
				var46 = var24 + var30;
				var47 = arg0 + arg2 >> 1;
				var48 = arg5 + arg6 >> 1;
				var49 = arg9 + arg14 >> 1;
			} else if (var44 == 12) {
				var45 = var23 + var29;
				var46 = var22 + var30;
				var47 = arg0 + arg3 >> 1;
				var48 = arg6 + arg11 >> 1;
				var49 = arg1 + arg14 >> 1;
			} else if (var44 == 13) {
				var45 = var23 + var29;
				var46 = var23 + var30;
				var47 = arg3;
				var48 = arg11;
				var49 = arg1;
			} else if (var44 == 14) {
				var45 = var24 + var29;
				var46 = var23 + var30;
				var47 = arg13;
				var48 = arg8;
				var49 = arg17;
			} else if (var44 == 15) {
				var45 = var24 + var29;
				var46 = var24 + var30;
				var47 = arg2;
				var48 = arg5;
				var49 = arg9;
			} else {
				var45 = var23 + var29;
				var46 = var24 + var30;
				var47 = arg0;
				var48 = arg6;
				var49 = arg14;
			}
			this.field902[var31] = var45;
			this.field903[var31] = var47;
			this.field904[var31] = var46;
			var27[var31] = var48;
			var28[var31] = var49;
		}
		int[] var32 = field926[arg12];
		int var33 = var32.length / 4;
		this.field908 = new int[var33];
		this.field909 = new int[var33];
		this.field910 = new int[var33];
		this.field905 = new int[var33];
		this.field906 = new int[var33];
		this.field907 = new int[var33];
		if (arg15 != -1) {
			this.field911 = new int[var33];
		}
		int var34 = 0;
		for (int var35 = 0; var35 < var33; var35++) {
			int var40 = var32[var34];
			int var41 = var32[var34 + 1];
			int var42 = var32[var34 + 2];
			int var43 = var32[var34 + 3];
			var34 += 4;
			if (var41 < 4) {
				var41 = var41 - arg19 & 0x3;
			}
			if (var42 < 4) {
				var42 = var42 - arg19 & 0x3;
			}
			if (var43 < 4) {
				var43 = var43 - arg19 & 0x3;
			}
			this.field908[var35] = var41;
			this.field909[var35] = var42;
			this.field910[var35] = var43;
			if (var40 == 0) {
				this.field905[var35] = var27[var41];
				this.field906[var35] = var27[var42];
				this.field907[var35] = var27[var43];
				if (this.field911 != null) {
					this.field911[var35] = -1;
				}
			} else {
				this.field905[var35] = var28[var41];
				this.field906[var35] = var28[var42];
				this.field907[var35] = var28[var43];
				if (this.field911 != null) {
					this.field911[var35] = arg15;
				}
			}
		}
		int var36 = arg3;
		int var37 = arg13;
		if (arg13 < arg3) {
			var36 = arg13;
		}
		if (arg13 > arg13) {
			var37 = arg13;
		}
		if (arg2 < var36) {
			var36 = arg2;
		}
		if (arg2 > var37) {
			var37 = arg2;
		}
		if (arg0 < var36) {
			var36 = arg0;
		}
		if (arg0 > var37) {
			var37 = arg0;
		}
		int var38 = var36 / 14;
		int var39 = var37 / 14;
	}
}

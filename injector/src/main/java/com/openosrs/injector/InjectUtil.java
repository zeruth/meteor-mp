...
public interface InjectUtil
{
	...
	{
		return data.deobfuscated.findClass(data.classMap.get(apiMethod.getClazz().getName().replace(API_BASE, "")));
	}
	...
	static ClassFile getVanillaClassFromAnnotationString(InjectData data, Annotation annotation)
	{
		...
		return data.vanilla.findClass(data.classMap.get(str.replace(API_BASE, "")));
	}

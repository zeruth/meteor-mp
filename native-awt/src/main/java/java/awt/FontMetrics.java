...
public abstract class FontMetrics implements Serializable {
    ...
    public int charWidth(int ch) {
        char character = (char) ch; // Convert int to char
        return charWidth(character);
    }

    public int charWidth(char ch) {
        if (font == null) {
            throw new IllegalStateException("Font must be set before calling charWidth");
        }

        // Get the metrics for the given character
        Rectangle2D bounds = font.getStringBounds(Character.toString(ch), getFontRenderContext());
    ...
    private FontRenderContext getFontRenderContext() {
        // You'll need to provide the appropriate context; here's an example:

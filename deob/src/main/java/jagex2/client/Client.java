...
import com.meteor.rs2.StartPrivNetworkTask;
...
/*			signlink.startpriv(InetAddress.getByName("localhost"));
			...
			new StartPrivNetworkTask().execute();
    ...
    public final void initApplication(@OriginalArg(1) int width, @OriginalArg(0) int height) {
		...
		this.graphics = frame.graphics;
		this.drawArea = new PixMap(this.screenWidth, this.screenHeight);
					...
					if (this.showSocialInput) {
						...
						if (key == 13 || key == 10) {
							...
							if (this.socialAction == 3 && this.socialInput.length() > 0) {
								...
								this.socialInput = this.socialInput; //WordFilter.filter(this.socialInput);
						...
						if ((key == 13 || key == 10) && this.chatTyped.length() > 0) {
							...
							if (this.chatTyped.startsWith("::")) {
								...
								this.chatTyped = this.chatTyped; //WordFilter.filter(this.chatTyped); TODO: Fix
...
/*		if (signlink.mainapp != null) {
	...
	protected void load() {
		...
		try {
			...
			//TODO: Investigate if this bugfix changes anything
			...
			//WordFilter.unpack(wordenc); TODO: Fix
	...
	public void projectFromGround(@OriginalArg(1) int x, @OriginalArg(3) int height, @OriginalArg(0) int z) {
...
/*	@OriginalMember(owner = "client!client", name = "getParameter", descriptor = "(Ljava/lang/String;)Ljava/lang/String;")
	...
	private void loadTitleBackground() {
		...
		@Pc(14) Pix24 title = new Pix24(data);

...
public class World {
	...
	private static int interpolate(@OriginalArg(0) int a, @OriginalArg(1) int b, @OriginalArg(2) int x, @OriginalArg(3) int scale) {
		@Pc(11) int f = 65536 - Pix3DMapView.cosTable[x * 1024 / scale] >> 1;
					...
					for (int z0 = -5; z0 < this.maxTileZ + 5; z0++) {
						...
						if (z0 >= 1 && z0 < this.maxTileZ - 1 && (!lowMemory || (this.levelTileFlags[level][x0][z0] & 0x10) == 0 && this.getDrawLevel(level, x0, z0) == levelBuilt)) {
							...
							if (underlayId > 0 || overlayId > 0) {
								...
								int shadeColor = 0;
								if (baseColor != -1) {
									shadeColor = Pix3DMapView.colourTable[mulHSL(tintColor, 96)];
								}
								...
								if (overlayId == 0) {
									...
									if (textureId >= 0) {
										rgb = Pix3DMapView.getAverageTextureRGB(textureId);
										hsl = -1;
										...
										rgb = Pix3DMapView.colourTable[this.adjustLightness(flo.hsl, 96)];
									}
